...
def get_parser():
    parser = argparse.ArgumentParser(
        prog="ecdsa-bitcoin-attack",
        description="Run the polynomial nonce recurrence attack on a Bitcoin dataset."
    )
    parser.add_argument("--input", "-i", type=str, required=True,
                        help="Path to the input file. Must be a dump file in the format produced by ecdsa-dump-bitcoin "
    ...
    with executor as ex:
        with open(input_path, "r") as f:
            for line in f:
                ...
                r, s, pubkey, txid, message_hash, block_timestamp = fields
...
def process_batch(current_batch):
    ...
    for entry in current_batch:
        r, s, pubkey, txid, h, block_timestamp = entry
    ...
    pubkey_address = first_pubkey.address()
